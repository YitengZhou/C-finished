...
void Order(Hash *h,char *str)
{
  ...
  if (h->b[key].number==0){
    ...
    p->next=(Block*) calloc(1, sizeof(Block));
    strcpy(p->next->word,str);
    p->next->number=GetKey(str);
    h->count++;
  }
}
...
void PrintHash(Hash *h)
{
  ...
  Block *p;
  for (i=0;i<h->size;i++){
    printf("%d:%s(%d)",i,h->b[i].word,h->b[i].number);
    p=&h->b[i];
    while (p->next!=NULL){
      printf("  ->  %d:%s(%d)",i,p->next->word,p->next->number);
      p=p->next;
    }
    printf("\n");
  }
