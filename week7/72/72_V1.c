...
void Creat_board(Board *queue,int parent,int number);
...
void Performance(char *string)
{
  ...
  char t[ROW][ROW] = {{'1','2','3'},{'4','5','6'},{'7','8',' '}};
  char step[ROW];
  ...
  do{
     ...
     for (i=0;i<3;i++){
       for (j=0;j<3;j++){
         ChangeColor(sw,t[i][j]);
         rectangle.x = ORIGIN + j*(GAP+RECTSIZE);
         rectangle.y = TARGET + i*(GAP+RECTSIZE);
         SDL_RenderFillRect(sw.renderer, &rectangle);
         Neill_SDL_DrawChar(&sw,fontdata,t[i][j],
...
void Creat_board(Board *queue,int parent,int cnt)
{
  int i,j;
  for (i=0;i<ROW;i++){
    for (j=0;j<ROW;j++){
      queue[cnt].arr[i][j]=queue[parent].arr[i][j];
    }
  }
  queue[cnt].number=cnt;
  queue[cnt].parent=parent;
}
...
/* test function Initial with correct initial board */
  ...
  assert(test[0].number==0);
  assert(strcmp(test[0].str,"1234 5678")==0);
