...
/* Some definitions of double hashing with separate chaining :
  hashkey1 is 300007 (prime and about double number of words),
  hashkey2 is 150011 (prime and greater than number of words),
  hashstandard means how many characters used to calculate key,
  alphabet means 26 letters */
#define HASHKEY1 300007
#define HASHKEY2 150011
#define HASHSTANDARD 5
#define ALPHABET 26.0
#define STRSIZE 1000
...
void mvm_insert(mvm* m, char* key, char* data)
{
  ...
  key1=GetKey1(key);
  /* Insert key/value pair without collisions */
  if (m->block[key1].index==0){
    InsertBlock(&m->block[key1],key,data);
    m->numkeys++;
  }
  else{
    while (flag==0){
      /* Insert key/value pair to separate chaining
        when key is completely same  */
      ...
      /* Insert key/value pair to double hashtable
        when collisions happened */
      else{
        i++;
        key1=(GetKey1(key)+i*GetKey2(key))%HASHKEY1;
        if (m->block[key1].index==0){
          InsertBlock(&m->block[key1],key,data);
          m->numkeys++;
          flag=1;
...
char* mvm_print(mvm* m)
{
  ...
  /* store strings from double hash and separate chaining */
...
/* Remove one key/value and free cell */
  ...
  while (flag==0){
    if (m->block[key1].index==0){
      ...
      if (strcmp(key,m->block[key1].key)==0){
        /* when delete key/value on the double hashtable */
        if (m->block[key1].same==NULL){
          mvmcell_free(&m->block[key1]);
        }
        /* when delete key/value on the separate chaining */
        else{
          cell=&m->block[key1];
          temp=cell->same;
...
void mvm_free(mvm** p)
{
  ...
  for (i=0;i<a->hashsize;i++){
    /* free key/data on the double hashtable */
    if (a->block[i].index!=0){
      free(a->block[i].key);
      free(a->block[i].data);
    }
    /* free key/data on the separate chaining */
    if (a->block[i].same!=NULL){
      cur=a->block[i].same;
      while (cur!=NULL){
        temp=cur;
        cur=cur->same;
...
{
  cell->key=(char *)calloc(strlen(key)+1,sizeof(char));
  cell->data=(char *)calloc(strlen(data)+1,sizeof(char));
  strcpy(cell->key,key);
  strcpy(cell->data,data);
  cell->index=GetKey(key);
}
...
void InsertSame(mvmcell* cell, char* key, char* data)
{
  ...
  InsertBlock(temp->same,key,data);
  return;
}

/* Getkey(key)=X4*26^4 + X3*26^3 + X2*26^2 + X1*26^1 + X0*26^0
  for example: abs('A'-'a')=32; abs(' '-'a')=-65;
  abs('1'-'a')=-48; abs('a'-'a')=0.
  so RYHME = 15*26^4 + 8*26^3 + 25*26^2 + 20*26^1 + 28*26^0
  so R AY1 M = 15*26^4 + 65*26^3 + 32*26^2 + 8*26^1 + 48*26^1 */
int GetKey(char *str)
{
  int i,length=strlen(str);
  double sum=0;
  if (length>HASHSTANDARD){
    length=HASHSTANDARD;
  }
  for (i=0;i<length;i++){
    sum+=(double)abs((str[i]-'a'))*pow(ALPHABET,length-i-1);
  }
  return (int)sum;
}

/* Getkey1(key) = Getkey(key) % HASHKEY1(300007) */
int GetKey1(char *str)
{
  return GetKey(str)%HASHKEY1;
}

/* Getkey2(key) = HASHKEY2(150011) - Getkey(key) % HASHKEY2 */
...
void PrintHashtable(mvm* m)
{
  ...
  for (i=0;i<m->hashsize;i++){
    if (m->block[i].index!=0){
      printf("\nblock[%d] : ",i);
      printf("[%s](%s){%d} ",m->block[i].key,\
              m->block[i].data,m->block[i].index);
      current=m->block[i].same;
      while (current!=NULL){
        printf("-> [%s](%s){%d} ",current->key,
              current->data,m->block[i].index);
        current=current->same;
      }
    }
  }
