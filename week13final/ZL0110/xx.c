...
void jump(Prog *p)
{
   ...
   /*printf("number = %d，jump to :%s\n", number, p->codes[p->cur]);*/
   instrs(p);
...
void compare(Prog *p)
{
   ...
   insertQueue('{', &q);
   /*printf("\ncmp_str1[%s] = %d, cmp_str2[%s] = %d\n", cmp_str1, cmp_result1, cmp_str2, cmp_result2);
   printf("CODE = %s\n", p->codes[p->cur+1]);*/
   ...
   /*printf("p->codes[%d] = %s\n", p->cur+5, p->codes[p->cur+5]);
   printf("before: q.back = %d, q.front = %d, number = %d\n", q.back, q.front, number);*/
      while (q.back - q.front != number) {
         while (!strsame(p->codes[p->cur], "}")) {
            if (strsame(p->codes[p->cur], "IFEQUAL") || strsame(p->codes[p->cur], "IFGREATER")) {
               insertQueue('{', &q);
               flag = 1;
               /*printf("in:     q.back = %d, q.front = %d, diffrence = %d\n", q.back, q.front, q.back - q.front);
                printf("insert ! before[%d] = %s\n", p->cur+8, p->codes[p->cur+8]); */
            }
            p->cur++;
           /* printf("p->cur = %d\n", p->cur); */
         }
         ...
         removeQueue(&q);
         /*printf("after:  q.back = %d, q.front = %d, diffrence = %d\n", q.back, q.front, q.back - q.front);
          printf("pop = %c\n", (char)pop(&s)); */

      }

      /*printf("after code[%d] = %s\n", p->cur, p->codes[p->cur]); */
}
...
int jumpPosition(Prog *p, int number)
{
   ...
   /*printf("str = |%s|\n", p->codes[0]);
   printf("1:  number = %d, i = %d, cnt = %d\n", number, i, cnt);*/
   while (cnt <= number) {
      /*printf("前:  code[%d] = |%s|, i = %d, cnt = %d\n", i, p->codes[i], i, cnt);*/
      ...
      /*printf("后:  code[%d] = |%s|, i = %d, cnt = %d\n", i, p->codes[i], i, cnt);*/
   }
   /*printf("4:  number = %d, i = %d, cnt = %d\n", number, i, cnt);*/
