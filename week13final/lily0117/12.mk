...
SFLAGS =   -fsanitize=undefined -g3 -lm
...
parse : $(BASE).c $(BASE).h fmvm.c fmvm.h
	$(CC) $(BASE).c fmvm.c $(CFLAGS) -o $@
parse_s : $(BASE).c $(BASE).h fmvm.c fmvm.h
	$(CC) $(BASE).c fmvm.c $(SFLAGS) -o $@
parse_d : $(BASE).c $(BASE).h fmvm.c fmvm.h
	$(CC) $(BASE).c fmvm.c $(DFLAGS) -o $@

# I #define INTERP (via this makefile) so that only the extra code for
# the interpreter is inside #ifdef INTERP / #endif 'brackets'
interp : $(BASE).c $(BASE).h fmvm.c fmvm.h
	$(CC) $(BASE).c fmvm.c $(CFLAGS) -o $@ -DINTERP
interp_s : $(BASE).c $(BASE).h fmvm.c fmvm.h
	$(CC) $(BASE).c fmvm.c $(SFLAGS) -o $@ -DINTERP
interp_d : $(BASE).c $(BASE).h fmvm.c fmvm.h
	$(CC) $(BASE).c fmvm.c $(DFLAGS) -o $@ -DINTERP
